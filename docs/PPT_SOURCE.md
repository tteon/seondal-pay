...
🎯 **"주장이 아니라 측정 — 사전 등록 실험과 라이브 온체인 결제"**

### 실험적 증명 (Kimi K3, 조걳별 빌링 키 분리, 사전 등록 기준)

**실험 1 — 온톨로지 효용성 (200샘플, ✅ PASS)**
| | 토큰 | 정확도 | 환각 | 비용 |
|---|---|---|---|
| 원시 텍스트 | 231,325 | 91~97% | 3~9% | 기준 |
| **온톨로지 노드** | **−81%** | **98~100%** | **0%** | **−40~71%** |

**실험 2 — 멀티에이전트 유의미성 (OpenAI Agents SDK)**
| 조건 | 정확도 | 토큰 | 판정 |
|---|---|---|---|
| A: 통합 에이전트 | 90% | 1× | 기준 |
| B: 역할 분해 (자유 텍스트 핸드오프) | 50% | 2.74× | ❌ 순진한 멀티에이전트는 악화 |
| C: 역할 분해 (**타입드 JSON 핸드오프**) | **90%** | 4.23× | ✅ 타입드 경계만이 회복 |
...
- **관찰성 스택 가동**: Prometheus가 seondal-pay 스크래이프 중(2/2 up), Grafana 대시보드, Loki/Tempo/Collector
- **알림 룰 7종** + **프로토콜 테스트 7종 통과** + **GitOps 자기치유**
